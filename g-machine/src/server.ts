...
io.on('connection', (socket) => {
  ...
  socket.on('join_game', ({ userId, gameSlug }) => {
    console.log(`User ${userId} joined game ${gameSlug}`);
  ...
  socket.on('spin', async ({ userId, bet, clientSeed, nonce }) => {
    try {
      ...
      console.log(`Spin requested: User=${userId}, Bet=${bet}`);
      ...
      try {
        ...
        console.error('Wallet sync error:', syncError.response?.data || syncError.message);
        socket.emit('error', { 
  ...
  socket.on('disconnect', () => {
    ...
    console.log('Client disconnected from G-Machine');
  });
});

app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'g-machine' });
});
