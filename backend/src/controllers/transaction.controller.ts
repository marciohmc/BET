...
export const processGameBet = async (req: AuthRequest, res: Response) => {
  try {
    const { amount, gameId, secret } = req.body;
    
    // Validate G-Machine Secret
    if (secret !== process.env.G_MACHINE_SECRET) {
      return res.status(401).json({ message: 'Unauthorized G-Machine request' });
    }
    ...
    if (!updatedUser) {
      return res.status(400).json({ message: 'Insufficient balance or race condition' });
    }
    ...
    const transaction = new Transaction({
      ...
      transactionId: `bet_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`
    });
...
export const processGameWin = async (req: AuthRequest, res: Response) => {
  try {
    const { amount, gameId, secret, betTransactionId } = req.body;
    ...
    const transaction = new Transaction({
      ...
      balanceBefore,
      balanceAfter: updatedUser.balance,
      description: `Win on game: ${gameId}`,
      transactionId: `win_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`
    });
